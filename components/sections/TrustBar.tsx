...
const badges = [
  ...
  { Icon: Clock, label: '24/7 Emergency', sub: '30-min arrival' },
  { Icon: DollarSign, label: 'No Trip Fee', sub: 'Free estimates' },
  { Icon: ShieldCheck, label: 'Since 2020', sub: 'Local specialists' },
];

export default function TrustBar() {
  return (
    <section
      className="border-y border-white/5 bg-brand-dark/50 py-6"
          ...
          {badges.map((b) => (
            <div key={b.label} className="flex items-center gap-3">
              <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-brand-teal/10">
                <b.Icon className="h-4 w-4 text-brand-teal" aria-hidden="true" />
              </div>
              <div>
