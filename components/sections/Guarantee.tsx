...
const promises = [
  {
    ...
    description:
      'If our advanced detection technology doesn\u2019t locate your leak, the inspection is on us. Zero risk, zero pressure.',
  },
  {
    Icon: DollarSign,
  ...
  },
  {
    ...
    description:
      'We commit to a 30-minute arrival for emergencies in Ballantyne. If we\u2019re late, your emergency call is free.',
  },
];
...
function PromiseCard({ promise, index }: { promise: (typeof promises)[number]; index: number }) {
  ...
  return (
    <div
      ref={ref}
      className={`rounded-2xl border border-brand-teal/15 bg-white/[0.03] p-6 transition-all duration-500 hover:border-brand-teal/30 hover:bg-white/[0.05] ${
        inView ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'
      }`}
      style={{ transitionDelay: `${index * 100}ms` }}
    >
      <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-xl bg-brand-teal/10 ring-1 ring-brand-teal/20">
        <promise.Icon className="h-6 w-6 text-brand-teal" aria-hidden="true" />
      </div>
